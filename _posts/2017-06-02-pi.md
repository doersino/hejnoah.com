---
layout:       post
title:        "Advanced SQL: Approximating Pi Using the Monte Carlo Method in Postgres"
...
$$\begin{align}
\frac{A}{B} &\approx \frac{m}{n}\\
\Rightarrow \frac{\pi r^2}{4 r^2} &\approx \frac{m}{n}\\
...
The visualizations in the "theory" section above have been drawn in LaTeX/Ti*k*Z[^pdf2svg], including the random dots used to approximate $$\pi$$ in the SQL query.

At some point I realized that it's not too hard to add a counter to the `\foreach` loop responsible for placing the dots, making it possible to keep track of how many dots fall within the bounds of the circle. Computing the corresponding approximation for $$\pi$$ works as above and showing it as part of the drawing was trivial:

{:refdef: style="text-align: center;"}
![]({{ "/static/pi4.svg" | relative_url }})
{: refdef}

Simply adjust the random seed `3455632` and optionally the iteration count `\i` in the LaTeX document below, compile[^luatex] and you'll observe a different distribution of the dots and, most likely, a slightly different approximation of $$\pi$$ below the plot.
...
\begin{tikzpicture}[scale=3.5]
    ...
    \foreach \p in {1,...,\i} {
        ...
        \ifdim\dist pt < 0.25pt
            ...
            \fill[fill=gray!50!black] (\x,\y) circle (0.25pt);
        \else
            \fill[fill=gray!50!white] (\x,\y) circle (0.25pt);
        \fi
    }
    \pgfmathparse{(4*\thepoints/\i)}
    \node[below of=m,yshift = -1.6cm] {$\pi \approx \frac{\thepoints}{\i} \cdot 4 \approx \pgfmathresult$};
